...
for i in range(len(matrixAlternativesExpertOne[0][len(matrixAlternativesExpertOne[i]) - 1])):
   ...
   for j in range(len(matrixAlternativesExpertOne)):
      maxAlternative = matrixAlternativesExpertOne[j][len(matrixAlternativesExpertOne[j]) - 1][i] if matrixAlternativesExpertOne[j][len(matrixAlternativesExpertOne[j]) - 1][i] > matrixAlternativesExpertTwo[j][len(matrixAlternativesExpertTwo[j]) - 1][i] else matrixAlternativesExpertTwo[j][len(matrixAlternativesExpertTwo[j]) - 1][i]
...
print(
   "Найкращий варіант - ",
   str(number),
   ". Максимальна функція корисності = ",
   str(result),
)